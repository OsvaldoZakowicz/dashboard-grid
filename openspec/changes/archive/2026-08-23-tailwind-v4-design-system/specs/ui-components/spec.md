...
- THEN no existe ningún elemento `<id>` inválido (el ícono de mail de Feather usa un elemento válido)
...
### Requirement: UC-3 — Sidebar drawer
...
El ítem de la lista de notificaciones MUST seguir el patrón: ícono (Feather) + texto del mensaje + link de acción. Copy de ejemplo en español: `Tu informe semanal está listo.` con acción `Ver informe`. El ícono MUST ser decorativo (`aria-hidden="true"`).

#### Scenario: Íconos decorativos ocultos al AT

- GIVEN un ítem de notificación con ícono Feather
...
### Requirement: UC-10 — Restricción ícono Feather × Alpine

Cada componente que renderice íconos Feather dentro de regiones renderizadas por Alpine (`x-for`, `x-if` o nodos renderizados tardíamente) MUST incluir una mitigación para que los íconos aparezcan: re-ejecutar `feather.replace()` después de los hooks de render de Alpine, o usar SVG inline. Ejecutar `feather.replace()` una sola vez en la carga NO es suficiente para nodos renderizados por Alpine. Los tamaños de ícono MUST seguir la escala documentada (sm 16 / md 18 / lg 24).
...
- THEN cada ícono aparece como SVG (no un tag `<i>` crudo)
