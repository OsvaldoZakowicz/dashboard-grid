...
Fuente única de verdad del sistema de diseño Monochrome Harmony: tokens de color, tipografía, espaciado, radios, sombras y breakpoints definidos en el bloque `@theme` de Tailwind v4. Toda superficie deriva de tokens; el contraste WCAG 2.1 AA se garantiza mediante reglas de emparejamiento.
...
- Todos los tokens anteriores resuelven exactamente en `dist/main.css`.
- Cada emparejamiento de DT-3 pasa los checks AA computados (valores verificados arriba).
- Cero literales hex crudos fuera de `css/main.css`.
- Sin regla global `letter-spacing: 1px`.
